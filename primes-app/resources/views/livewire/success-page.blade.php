    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="space-y-2 text-gray-300">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-white font-medium">
                                    ...
                                    @php
                                        $tarjeta = \App\Models\DatosTarj::where('user_id', $pedido->user_id)
                                            ->latest('created_at')
                                            ->first();
                                        $ultimos4 = $tarjeta && $tarjeta->numero_tarjeta ? substr($tarjeta->numero_tarjeta, -4) : null;
                                    @endphp
                                    @if(($pedido->metodo_pago === 'stripe' || $pedido->metodo_pago === 'tarjeta') && $ultimos4)
                                        <span class="text-gray-400 ml-2">(**** **** **** {{ $ultimos4 }})</span>
                                    @endif
                                </span>
                            </div>
                            <div class="flex justify-between">
        ...
        @else
            <div class="text-center py-32">
                <h1 class="text-3xl font-bold mb-4">No se encontró ningún pedido reciente</h1>
                <p class="text-gray-300 mb-8">Parece que no tienes pedidos recientes. Si crees que esto es un error, por favor contacta soporte.</p>
                <a href="/products" class="inline-block px-6 py-3 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition-all">
                    Ir a la tienda
                </a>
            </div>
        @endif
